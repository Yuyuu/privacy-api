...
class Bus(object):
    ...
    def send_and_wait_response(self, message):
        ...
        if len(handlers) == 0:
            logger.warning('Impossible to find a handler for %s', message.__class__.__name__)
            return ExecutionResult.error(BusError('Impossible to find a handler for current message'))

        logger.debug('New message: %s', message.__class__.__name__)
        results = []
        for handler in handlers:
    ...
    def _execute(message, handler):
        try:
            logger.debug('Executing handler %s', handler.__class__.__name__)
